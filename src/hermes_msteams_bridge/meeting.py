...
async def _deliver_to_teams(conversation_id: str, text: str) -> bool:
    ...
    if not result.get("success"):
        logger.warning(
            "[msteams_bridge] minutes delivery to %s failed: %s",
...
def _save_docx_artifact(minutes: str) -> str | None:
    ...
    try:
        ...
        logger.info("[msteams_bridge] minutes document saved: %s", path)
        return str(path)
    except Exception:  # noqa: BLE001 — artifact is optional
        logger.warning("[msteams_bridge] minutes .docx generation failed", exc_info=True)
        return None
...
async def post_minutes(
    ...
    try:
        ...
        logger.error("[msteams_bridge] meeting summary failed", exc_info=True)
