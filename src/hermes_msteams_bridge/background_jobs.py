...
def _jobs_dir():
    try:
        ...
        d = hermes_home() / "cache" / "msteams_bridge" / "jobs"
...
def job_create(query: str, thread_id: str, session_key: str = "") -> str | None:
    ...
    try:
        ...
        logger.warning("[msteams_bridge] job persist failed; running non-durable", exc_info=True)
        return None
...
def claim_pending_jobs() -> list[dict]:
    ...
    for f in sorted(d.glob("*.json")):
        if len(claimed) >= _RESUME_MAX:
            logger.info("[msteams_bridge] resume cap reached; remaining jobs stay queued for the next cycle")
            break
        try:
        ...
        if age > _JOB_TTL_S:
            logger.info("[msteams_bridge] dropping stale background job (%.0f min old)", age / 60)
            f.unlink(missing_ok=True)
            continue
        if not entry.get("thread_id"):
            logger.info("[msteams_bridge] dropping resumed job with no delivery target")
            f.unlink(missing_ok=True)
            continue
...
async def resume_pending_jobs() -> int:
    ...
    for job in claim_pending_jobs():
        ...
        logger.info("[msteams_bridge] resuming interrupted background job: %.60s", query)
        consult = AgentConsult(session_id=session_key or None)
        try:
            result = await consult.ask(query, timeout_s=300.0)
        except Exception:  # noqa: BLE001
            logger.error("[msteams_bridge] resumed job failed", exc_info=True)
        ...
        if ok:
            ...
            logger.warning(
                "[msteams_bridge] resumed job delivery failed (kept for retry): %s",
                outcome.get("error"),
            )
    return delivered
