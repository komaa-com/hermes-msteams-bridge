...
flowchart TD
    ...
    Plugin["teams_call plugin (this repo)<br/>bridge_server.py: WS server, auth + lifecycle<br/>handlers.py: the call brain<br/>call_session_base.py: shared call state"]
...
| `cli.py`, `__init__.py`, `tools.py` | `hermes teams-call serve|status`, plugin registration, the `teams_call_status` tool. |
