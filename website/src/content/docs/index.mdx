...
package `hermes_msteams_bridge`, Hermes plugin name `msteams_bridge`) adds **Microsoft
...
- **Meetings** - "speak only when addressed" group etiquette, per-speaker minutes,
  ...
  `.docx` attached to the chat as a file card, plus a local artifact).
...
flowchart LR
    ...
    Plugin["msteams_bridge plugin (in Hermes)<br/>the local WS server + the call brain"]
...
The plugin is the **local WebSocket server** (binds `127.0.0.1:9442`). StandIn is
